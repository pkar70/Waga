...
#End Region
    ...
    Private Shared Async Function SendAsPodejscie3(sMAC As String, oBuff As Windows.Storage.Streams.IBuffer, bMsg As Boolean) As Task(Of Boolean)
        ...
        If Not oDevice.DeviceInformation.Pairing.IsPaired Then
            ...
            If Not bMsg Then Return False
            Try
                Await oDevice.DeviceInformation.Pairing.PairAsync()
            Catch ex As Exception
                DebugOut("SendAsPodejscie3, pairing exception")
        ...
        Catch ex As Exception
            ...
            CrashMessageAdd("SendAsPodejscie3, GetRfcommServicesForIdAsync", ex, True)
            oSvcRes = Nothing
        End Try
        ...
        Else
            ...
            bRet = Await SendAsPodejscie3Socket(oSocket, oService, oBuff)
            oSocket.Dispose()
            oService.Dispose()
        End If

        DebugOut("SendAsPodejscie3, disposing")
        oDevice.Dispose()
    ...
    Private Shared Async Function SendAsPodejscie3Socket(oSocket As Windows.Networking.Sockets.StreamSocket, oService As Windows.Devices.Bluetooth.Rfcomm.RfcommDeviceService, oBuff As Windows.Storage.Streams.IBuffer) As Task(Of Boolean)
        Try
        ...
        Catch ex As Exception
            DebugOut("SendAsPodejscie3, ConnectAsync exception")
        ...
        Catch ex As Exception
            DebugOut("SendAsPodejscie3, WriteAsync exception")
...
#If DEBUG Then
        If GetSettingsBool("LogEnabled") Then
            Dim oFile As Windows.Storage.StorageFile = Await GetLogFileMonthlyAsync("", "")
            Await oFile.AppendLineAsync(Date.Now.ToString("yyyy.MM.dd HH:mm:ss") & vbTab & dPercent.ToString("0.00") & "%")
        End If
#End If
